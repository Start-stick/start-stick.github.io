...
export function WordPasterManager()
{
	...
	this.Config = {
		...
		, "ExePath": "http://res2.ncmem.com/download/WordPaster/fast/2.0.42/WordPaster.exe"
	...
	if (arguments.length > 0) {
        ...
        if (typeof (cfg.event) != "undefined") $.extend(true, this.data.event, cfg.event);
    }

	this.data.editor=this.Config.editor;
    ...
    this.app = {
		...
		, importWordToImg: function () {
			var param = { name: "importWordToImg" };
			this.postMessage(param);
		}
		, importWord: function () {
			var param = { name: "importWord"};
			this.postMessage(param);
		}
		, importExcel: function () {
			var param = { name: "importExcel"};
			this.postMessage(param);
		}
		, importPPT: function () {
			var param = { name: "importPPT"};
			this.postMessage(param);
		}
		, importPDF: function () {
    ...
    this.setup_tip = function () {
        ...
        dom.find("img[name='ok']").attr("src", "../../static/WordPaster/ok.png");
        dom.find("img[name='setup']").attr("src", "../../static/WordPaster/setup.png");
		_this.ui.setup.show();
	...
	{
		//https://www.tiny.cloud/docs/tinymce/5/apis/tinymce.editor/#execCommand
		//tinyMCE.activeEditor.execCommand('mceInsertRawHTML', false, html);
		this.data.editor.insertContent("<span>"+html+"</span>",{format:'html'});
	};
	...
	*/
	...
	this.Queue_Complete = function (json)
	{
	    ...
	    {
			var reg = /data-src="[^"]+"/gi;
			json.word=json.word.replace(reg,"");
			tinymce.activeEditor.setContent(json.word, {format: 'html'});
	    } //上传Word图片时才替换内容
	...
	//加载
    if (typeof (this.ui.render) == "undefined") 
	{
		$(function () {
			_this.LoadTo($(document.body));
		})
    }
	else if (typeof (this.ui.render) == "string") {
        $(function () {
